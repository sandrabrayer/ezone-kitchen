...
// Login codes are WORDS, matched case-insensitively and ignoring surrounding
// whitespace: `ramot`, `RAMOT`, and `" Ramot "` all match a stored `RAMOT`.
// Normalise both sides the same way, then compare in constant time.
...
function checkCode(input, expected) {
  ...
  if (!b) return false; // an empty stored code never authenticates
  if (a.length !== b.length) return false;
  return crypto.timingSafeEqual(Buffer.from(a), Buffer.from(b));
}

module.exports = { signToken, verifyToken, checkCode, normalizeCode, isRole, APP_SCOPE, ROLES, DEFAULT_DAYS };
