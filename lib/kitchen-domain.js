...
(function (root, factory) {
  ...
  /* ---------------------------------------------------------------- *
   * Split budgets — a house budget is TWO lines: the total monthly    *
   * food budget and, optionally, a separate מדריכים (instructors)     *
   * budget. Back-compat: a legacy record with only a total reads as   *
   * { budget, instructorsBudget: 0 }. Amounts are always non-negative. *
   * ---------------------------------------------------------------- */
  // Coerce any input to a non-negative amount (invalid / negative → 0).
  function nonNegativeAmount(v) {
    const n = Number(v);
    return Number.isFinite(n) && n > 0 ? n : 0;
  }

  // Validate & sanitise a budget upsert. Numbers are coerced to non-negative;
  // an instructors budget GREATER than the total is allowed but reported as a
  // WARNING (a house may over-commit its instructor line) — never an error. The
  // result is always { ok: true, value, warnings } so callers can persist the
  // sanitised value while surfacing warnings to the user.
  function validateBudgetInput(input) {
    input = input || {};
    const budget = nonNegativeAmount(input.budget);
    const overrun = nonNegativeAmount(input.overrun);
    const instructorsBudget = nonNegativeAmount(input.instructorsBudget);
    const warnings = [];
    if (instructorsBudget > budget && budget > 0) warnings.push('instructors_over_total');
    return {
      ok: true,
      value: {
        budget: round2(budget),
        overrun: round2(overrun),
        overrunNote: String(input.overrunNote == null ? '' : input.overrunNote),
        instructorsBudget: round2(instructorsBudget),
      },
      warnings: warnings,
    };
  }
  ...
  /* ---------------------------------------------------------------- *
   * Instructor (מדריך) cost — the actual monthly labour cost of the   *
   * house's instructors, summed from its workers list. A worker is a  *
   * { name, role, cost, actuals } record; only workers whose role is  *
   * מדריך count. RECORDED monthly actuals (worker.actuals['YYYY-MM'])  *
   * win when any instructor has one; otherwise the figure is an        *
   * ESTIMATE — the sum of each instructor's standard monthly cost, or, *
   * when the house has no instructor workers at all, the house's       *
   * instructorsBudget — and is flagged { estimated: true } so the UI   *
   * shows the אומדן badge. Pure.                                        *
   * ---------------------------------------------------------------- */
  const INSTRUCTOR_ROLE = 'מדריך';

  function isInstructorRole(role) {
    return String(role == null ? '' : role).trim() === INSTRUCTOR_ROLE;
  }

  // A worker's recorded actual for a month, or null when none is recorded.
  function workerMonthlyActual(worker, month) {
    const actuals = worker && worker.actuals;
    if (actuals && Object.prototype.hasOwnProperty.call(actuals, month)) {
      const n = Number(actuals[month]);
      if (Number.isFinite(n) && n >= 0) return n;
    }
    return null;
  }

  function instructorCostForMonth(workers, month, fallbackBudget) {
    const instructors = (workers || []).filter(function (w) { return isInstructorRole(w && w.role); });
    let actualSum = 0;
    let estimateSum = 0;
    let hasActual = false;
    instructors.forEach(function (w) {
      const base = nonNegativeAmount(w && w.cost);
      estimateSum += base;
      const rec = workerMonthlyActual(w, month);
      if (rec != null) { hasActual = true; actualSum += rec; }
      else { actualSum += base; } // a month with no record falls back to the standard cost
    });
    if (hasActual) {
      return { cost: round2(actualSum), estimated: false, count: instructors.length };
    }
    const fallback = nonNegativeAmount(fallbackBudget);
    const cost = estimateSum > 0 ? estimateSum : fallback;
    return { cost: round2(cost), estimated: true, count: instructors.length };
  }

  // Utilisation % of a budget by a cost (0 when no budget). Rounded to whole %.
  function utilisationPct(cost, budget) {
    const b = nonNegativeAmount(budget);
    if (!(b > 0)) return 0;
    return Math.round((nonNegativeAmount(cost) / b) * 100);
  }
  ...
  function scaledPar(seedMin, unit, baseTotal) {
    const people = clampCount(baseTotal);
  ...
  function baselineForHouse(catalog, baseTotal, overrides) {
    ...
    return { baseTotal: people, rows: rows, total: total };
  }
  ...
  const SEED_CATALOG = [
    ...
    // בשר — meat & fish
    { name: 'עוף שלם/פרגיות', unit: 'kg', category: 'meat', min: 12, price: 22 },
    { name: 'חזה עוף', unit: 'kg', category: 'meat', min: 8, price: 32 },
    { name: 'כרעיים', unit: 'kg', category: 'meat', min: 8, price: 20 },
    { name: 'בשר טחון', unit: 'kg', category: 'meat', min: 6, price: 55 },
    { name: 'שניצל', unit: 'kg', category: 'meat', min: 6, price: 35 },
  ...
  return {
    ...
    // split budgets + instructor cost
    nonNegativeAmount: nonNegativeAmount,
    validateBudgetInput: validateBudgetInput,
    INSTRUCTOR_ROLE: INSTRUCTOR_ROLE,
    isInstructorRole: isInstructorRole,
    instructorCostForMonth: instructorCostForMonth,
