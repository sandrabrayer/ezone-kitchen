...
(function (root, factory) {
  ...
  /* A stock-count lists the WHOLE pantry: every catalog item PLUS any current
     stock item not in the catalog (free-text additions), grouped by category.
     The count is the full inventory summary — saving it rebuilds stock. */
  function countKey(name, category) {
    return (isCategory(category) ? category : 'groceries') + '::' + normName(name);
  }

  // The rows to show in a stock count: union(catalog, stock), each with the
  // item's unit, default min, current stock id (if any) and current qty
  // (0 when not yet stocked). Category-ordered, then by name.
  function buildCountList(catalog, stock) {
    const byKey = new Map();
    (catalog || []).forEach(function (c) {
      const name = String((c && c.name) || '').trim();
      if (!name) return;
      const category = isCategory(c && c.category) ? c.category : 'groceries';
      const key = countKey(name, category);
      if (!byKey.has(key)) {
        byKey.set(key, { key: key, name: name, category: category, unit: safeUnit(c && c.unit), min: sanitizeMin(c && c.min), stockId: null, qty: 0 });
      }
    });
    (stock || []).forEach(function (raw) {
      const it = readStockItem(raw);
      if (!it.name) return;
      const key = countKey(it.name, it.category);
      const cur = byKey.get(key);
      if (cur) {
        cur.stockId = (raw && raw.id) || cur.stockId;
        cur.qty = it.value;
        cur.unit = it.unit;
        if (!(cur.min > 0)) cur.min = it.min;
      } else {
        byKey.set(key, { key: key, name: it.name, category: it.category, unit: it.unit, min: it.min, stockId: (raw && raw.id) || null, qty: it.value });
      }
    });
  ...
  // Apply a completed count. `countedByKey` maps countKey → quantity. The result
  // IS the full pantry: existing stock items keep their identity and take the
  // counted qty (0 kept); catalog items counted > 0 that weren't stocked are
  // added; catalog items left at 0 are omitted. Pure — returns a NEW array.
  function applyStockCount(catalog, stock, countedByKey) {
    const stockById = new Map((stock || []).filter(function (s) { return s && s.id; }).map(function (s) { return [s.id, s]; }));
    const counted = countedByKey || {};
    const result = [];
    buildCountList(catalog, stock).forEach(function (row) {
      const has = Object.prototype.hasOwnProperty.call(counted, row.key);
      const raw = has ? Number(counted[row.key]) : row.qty;
      const qty = Number.isFinite(raw) && raw > 0 ? raw : 0;
      if (row.stockId && stockById.has(row.stockId)) {
        const it = readStockItem(stockById.get(row.stockId));
        result.push({ id: row.stockId, name: it.name, category: it.category, qty: qty, unit: it.unit, minQty: it.min });
      } else if (qty > 0) {
        result.push({ id: newId('stk'), name: row.name, category: row.category, qty: qty, unit: safeUnit(row.unit), minQty: sanitizeMin(row.min) });
      }
    });
    return result;
  }
