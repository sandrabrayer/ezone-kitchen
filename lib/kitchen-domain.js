/* ezone-kitchen — canonical, testable domain logic.
   ...
   calculation as a small pure function:
     ...
     aggregateWeek()    Σ over (day, meal, dish, ingredient) qtyPerPerson × people(day)
...
(function (root, factory) {
  ...
  /* Normalise a raw ingredient (menu) to { name, category, value, unit }.
     Back-compat: pre-unit data carried `qtyKgPerPerson` (kilograms). */
  ...
  /* ---------------------------------------------------------------- *
   * Week aggregation — Σ (qtyPerPerson in base unit) × people(day).  *
   * A line's quantity is carried in its family's BASE unit; lines of *
   * the same name+category but different families stay separate.     *
   * ---------------------------------------------------------------- */
  // Σ over the given days: (qtyPerPerson in base unit) × people(day), merged by
  // (category, name, unit-family). Returns lines in base units, category-ordered.
  function accumulateDays(week, headcount, days) {
      ...
      for (let mi = 0; mi < MEALS.length; mi++) {
        ...
        for (let i = 0; i < dishes.length; i++) {
          ...
          for (let j = 0; j < ings.length; j++) {
            ...
            const perPersonBase = toBaseValue(ing.value, ing.unit);
            const amount = perPersonBase * people;
  ...
  // Whole-week requirement (drives the shopping-list projection).
  function aggregateWeek(week, headcount) {
    return accumulateDays(week, headcount, DAYS);
  }

  // Actual consumption for ONE day — qtyPerPerson × people, NO purchasing
  // buffer. This is what a "בוצע" (mark-served) action deducts from the pantry.
  function dayConsumption(week, headcount, day) {
    if (DAYS.indexOf(day) === -1) return [];
    return accumulateDays(week, headcount, [day]);
  }
  ...
  // unit; stock is matched by name + unit family (kg↔g, l↔ml).
  ...
  function cloneIngredient(ing) {
    ...
    return { id: newId('ing'), name: norm.name, category: norm.category, qtyPerPerson: norm.value, unit: norm.unit };
  }
