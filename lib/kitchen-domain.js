...
(function (root, factory) {
   ...
   * cooked meal (lighter pantry food).                                  *
   *                                                                    *
   * REDUCED-OCCUPANCY WEEKEND — from FRIDAY MORNING until SUNDAY        *
   * MORNING fewer people are in the house the whole window, so ‎-25%    *
   * applies to ALL Friday meals (בוקר/צהריים/ערב) AND ALL Saturday      *
   * meals (בוקר/צהריים/ערב) — not only the planned weekend meals.       *
   * Sunday breakfast onward is back to full. This drives every         *
  ...
  // Reduced-occupancy weekend (Fri morning → Sun morning): 25% less product on
  // every Friday + Saturday meal.
  const WEEKEND_RATE = 0.75;
  const WEEKEND_DAYS = ['friday', 'saturday'];

  function isWeekendDay(day) {
    return WEEKEND_DAYS.indexOf(day) !== -1;
  }

  // A meal is COOKED/planned unless it is a self-serve evening. Self-serve
  // evenings are ערב on every day EXCEPT Friday (ליל שבת / Friday dinner is a
  // planned, cooked meal at the full count).
  function isSelfServeMeal(day, meal) {
    return meal === 'dinner' && day !== 'friday';
  }
  ...
  // The REFERENCE diner count for one (day, meal), for the תפריט meal header —
  // display only, it never scales the cook's entered quantities. A cooked meal
  // serves the full house; a self-serve evening serves מטופלים + 2. Friday +
  // Saturday are the reduced-occupancy window, so their count is 75% (rounded).
  function mealDiners(hc, day, meal) {
    const m = mealHeadcount(hc);
    const base = isSelfServeMeal(day, meal) ? m.evening : m.full;
    return isWeekendDay(day) ? Math.round(WEEKEND_RATE * base) : base;
  }

  // Daily consumption as a FRACTION of the full daytime count for a WEEKDAY
  // (Sun–Thu): two cooked meals at full + one self-serve evening (½ weight,
  // מטופלים+2 diners), over three meal slots. 1.0 would be three full cooked
  // meals. full === 0 → 0.
  ...
  // One (day, meal)'s consumption as a fraction of the full count: 1.0 for a
  // cooked meal, 0.5 × (evening/full) for a self-serve evening (both fewer diners
  // AND lighter fare), times the 0.75 weekend rate on Friday + Saturday. full ===
  // 0 → 0.
  function mealFactor(hc, day, meal) {
    const m = mealHeadcount(hc);
    if (!(m.full > 0)) return 0;
    const raw = isSelfServeMeal(day, meal) ? SELF_SERVE_MEAL_WEIGHT * (m.evening / m.full) : 1;
    return isWeekendDay(day) ? raw * WEEKEND_RATE : raw;
  }

  // Weekly consumption fraction of the full count, averaged over all 21 meal
  // slots (7 days × 3 meals) of the meal model. The reduced-occupancy weekend
  // (Fri morning → Sun morning) applies ‎-25% to every Friday + Saturday meal;
  // Sunday breakfast onward is back to full. Friday dinner is a cooked/planned
  // meal, so it counts as a FULL meal reduced 25% — not a self-serve evening.
  function weekFactor(hc) {
    const m = mealHeadcount(hc);
    if (!(m.full > 0)) return 0;
    let sum = 0;
    for (let d = 0; d < DAYS.length; d++) {
      for (let i = 0; i < MEALS.length; i++) sum += mealFactor(hc, DAYS[d], MEALS[i]);
    }
    return sum / (DAYS.length * MEALS.length);
  }
  ...
  // Normalise any month-ish value to a 'YYYY-MM' key: a Date, a 'YYYY-MM'
  // string, or a full ISO 'YYYY-MM-DD…' string. '' when unparseable. THE single
  // month-key helper shared by the "כל הבתים" overview and the תקציב tab (and the
  // batched backend endpoint) so all three agree on the key — the fix for the
  // overview showing ₪0 against a subtly different month string.
  ...
  // Actual spend for a calendar month, summed from purchase.date. The `month`
  // arg is normalised so a 'YYYY-MM' or a full ISO date both match.
  function actualSpendForMonth(purchases, month) {
    const mk = normMonth(month);
    return round2((purchases || [])
      .filter(function (p) { return monthOf(p.date) === mk; })
  ...
  // Raw per-house monthly figures for the "כל הבתים" overview — exactly what the
  // batched backend endpoint returns AND what the client computes locally as a
  // fallback from already-loaded state. Display (remaining / over-budget) is left
  // to summariseBudget so that rule lives in ONE place. Pure.
  function houseMonthRaw(house, month) {
    const mk = normMonth(month);
    const h = house || {};
    const budgets = h.budgets || {};
    // Prefer the exact key, but tolerate a stored key that isn't exactly
    // normalised (stray space, single-digit month) so a subtle mismatch can't
    // silently show ₪0.
    let b = budgets[mk];
    if (!b) {
      const k = Object.keys(budgets).filter(function (key) {
        return key !== '__proto__' && key !== 'constructor' && key !== 'prototype';
      }).find(function (key) { return normMonth(key) === mk; });
      if (k) b = budgets[k];
    }
    b = b || {};
    const budget = Number(b.budget);
    const overrun = Number(b.overrun);
    return {
      id: h.id,
      name: h.name || '',
      month: mk,
      budget: round2(Number.isFinite(budget) && budget > 0 ? budget : 0),
      overrun: round2(Number.isFinite(overrun) && overrun > 0 ? overrun : 0),
      actual: actualSpendForMonth(h.purchases, mk),
      diners: baseTotal(h.headcount),
    };
  }
  ...
  return {
    ...
    isSelfServeMeal: isSelfServeMeal,
    mealHeadcount: mealHeadcount,
    mealDiners: mealDiners,
