...
(function (root, factory) {
  ...
  /* Apply a stock count. `values` maps a count-row key (normalised name) to the
     ...
     quantity. Rules (per spec):
       • an item already in stock is set to its counted qty (INCLUDING 0 — a
         counted-empty pantry item stays in the list at 0);
       • an item NOT in stock counted > 0 is ADDED (with the catalog's unit,
         category and default par);
       • an item NOT in stock left at 0 is omitted (nothing to add).
     The result IS the full pantry summary. Pure: returns a NEW stock array. */
  function applyStockCount(catalog, stock, values) {
    values = values || {};
    const out = [];
    stockCountRows(catalog, stock).forEach(function (r) {
      ...
      if (r.id != null) {
        out.push({ id: r.id, name: r.name, category: r.category, qty: roundQty(counted), unit: r.unit, minQty: r.min });
      } else if (counted > 0) {
        out.push({ id: newId('stk'), name: r.name, category: r.category, qty: roundQty(counted), unit: r.unit, minQty: r.min });
      }
    });
