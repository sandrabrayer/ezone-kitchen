...
(function (root, factory) {
  ...
  /* ---------------------------------------------------------------- *
   * Item catalog — a shared list of { name, unit, category } offered  *
   * as a dropdown everywhere a name is typed. Free text is allowed;   *
   * new names are merged in. Deduplicated by normalised name.         *
   * ---------------------------------------------------------------- */
  function catalogKey(name) { return normName(name); }

  // Merge `entries` into `catalog`, de-duplicating by normalised name
  // (first-seen wins, so an existing catalog entry is never overwritten by a
  // later one). Blank names are dropped. Returned sorted by Hebrew name.
  ...
  // null; used to derive an item's default unit + category from its name.
